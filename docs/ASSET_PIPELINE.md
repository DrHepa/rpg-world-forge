...
lookalike pairs are distinct, and every fixture string has deterministic
authored-design mask evidence in the `narrative-fixture-design-mask-evidence-v2`
domain. Pillow is used only as an explicit `ImageFont.Layout.BASIC`
non-authoritative smoke that proves the generated TTF loads and renders
nonblank fixture text; no release content hash depends on raster pixels or RAQM
availability. Its glyph design and generated bytes are dedicated under CC0-1.0;
the stdlib-only generator code remains MIT. Both assets are produced through
`procedural_offline`, reviewed, and licensed with exact opaque fixture evidence;
generic QA consumers retain sorted evidence hashes and do not parse the font
mask domain. The bounded integration matrix
