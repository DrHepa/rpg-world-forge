...
def _windows_handle_stat(handle: int, *, strict: bool = False) -> WindowsFileStat:
    ...
    if get_extended_information(
        ...
        basic_error = ctypes.get_last_error()
        if strict:
            raise OSError(
                f"Strict Windows FileBasicInfo is unavailable: {_windows_error(basic_error)}"
            )
    ...
    if get_extended_information(
        ...
        file_id_error = ctypes.get_last_error()
        if strict:
            raise OSError(
                f"Strict Windows 128-bit FileIdInfo is unavailable: {_windows_error(file_id_error)}"
            )
...
def windows_handle_file_stat(handle: int) -> WindowsFileStat:
    """Inspect one retained native Windows handle without reopening its pathname."""

    if _platform_name() != "nt":
        raise OSError("Native Windows file-handle inspection is unavailable")
    return _windows_handle_stat(handle)


def windows_handle_file_stat_strict(handle: int) -> WindowsFileStat:
    """Inspect a handle using mandatory FileBasicInfo and 128-bit FileIdInfo."""

    if _platform_name() != "nt":
        raise OSError("Strict native Windows file-handle inspection is unavailable")
    return _windows_handle_stat(handle, strict=True)
