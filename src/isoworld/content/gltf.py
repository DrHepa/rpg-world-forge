"""Safe, non-executing validation for bounded GLB runtime resources."""
...
def _reject_non_finite(value: str) -> None:
    raise _JSONNumberError(f"non-finite JSON number: {value}")
...
def _parse_finite_float(value: str) -> float:
    try:
        ...
        raise _JSONNumberError(f"invalid JSON number: {value}") from exc
        ...
        if descriptor is not None:
            try:
                ...
                if not note_cleanup_failure(
                    primary,
                    cleanup_error,
                    context="GLB descriptor cleanup",
                ):
                    raise GLBError(f"Could not close {path}: {cleanup_error}") from cleanup_error
